...
const vClickOutside = {
  mounted(
    el: HTMLElement & { _clickOutside?: (e: Event) => void },
    binding: { value: () => void },
  ) {
    ...
    amrSequences: [
      {
        isolateId: 'ISO-101',
        geneSymbol: 'bla',
        sequenceName: 'subclass B3 metallo-beta-lactamase',
        elementType: 'AMR',
      ...
      },
      {
        isolateId: 'ISO-101',
        geneSymbol: 'erm',
        sequenceName: '23S rRNA methyltransferase Erm',
        elementType: 'AMR',
    ...
    amrSequences: [
      {
        isolateId: 'ISO-102',
        geneSymbol: 'aac(3)-I',
        sequenceName: 'AAC(3)-I aminoglycoside',
        elementType: 'AMR',
    ...
    amrSequences: [
      {
        isolateId: 'ISO-103',
        geneSymbol: 'arsN1',
        sequenceName: 'arsinothricin N-acetyltransferase',
        elementType: 'STRESS',
...
// WGS quality counts
const wgsPassCount = computed(
  () => detail.value?.wgsMetrics.filter((w) => w.qualityStatus === 'Passed').length ?? 0,
)
...
const safetyRating = computed(() => {
  ...
  if (hasIntermediate) {
    return {
      ...
      detail:
        'Bacteria with reduced antibiotic susceptibility have been detected but no confirmed resistant strains. Standard precautions apply — avoid swallowing water and wash hands after contact.',
    }
  }
  return {
    level: 'LOW RISK',
    ...
    detail:
      'Current data shows no antibiotic-resistant bacteria at this location. Standard hygiene practices are still recommended when near river water.',
  }
})
</script>
      ...
      <header class="page-header">
        ...
        <div class="site-hero-card" v-click-outside="closeDropdown">
          ...
          <button class="hero-name-btn" @click="dropdownOpen = !dropdownOpen">
            ...
            <i
              class="pi pi-chevron-down hero-chevron"
              :class="{ 'hero-chevron--open': dropdownOpen }"
            ></i>
          </button>
          ...
          <div class="meta-pills">
            ...
            <span class="pill"
              ><i class="pi pi-compass"></i> {{ detail.site.latitude }},
              {{ detail.site.longitude }}</span
            >
            <span class="pill"
              ><i class="pi pi-database"></i> {{ detail.waterSamples.length }} samples</span
            >
            <span class="pill"
              ><i class="pi pi-microscope"></i> {{ detail.isolates.length }} isolates</span
            >
          </div>
        </div>
      </header>
      ...
      >
        ...
        <div class="safety-body">
          <div class="safety-header-row">
            <span class="safety-level" :style="{ color: safetyRating.color }">{{
              safetyRating.level
            }}</span>
      ...
      <div class="stat-grid">
        <div class="stat-card">
          ...
          <div class="stat-body">
            ...
            <span class="stat-value">{{
              new Set(detail.waterSamples.map((s) => s.tripIdentifier)).size
            }}</span>
          </div>
        </div>
        <div class="stat-card">
        ...
        <div class="stat-card">
          ...
          <div class="stat-body">
            ...
            <span class="stat-value"
              >{{ wgsPassCount }}<span class="stat-denom">/{{ wgsTotalCount }}</span></span
            >
          </div>
        </div>
        <div class="stat-card">
            ...
            <div v-for="marker in binaryProfileSummary" :key="marker.marker" class="marker-row">
              ...
              <div class="marker-dots">
                <span
                  ...
                  class="marker-dot"
                  :class="
                    iso.binaryTypingProfile[marker.marker as keyof BinaryTypingProfile]
                      ? 'dot--pos'
                      : 'dot--neg'
                  "
                  :title="iso.isolateId"
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="iso in detail.isolates" :key="iso.isolateId">
                ...
                <td class="organism-cell">
                  <em>{{ iso.organismIdentity }}</em>
                </td>
                ...
                <td>
                  <span
                    class="bool-badge"
                    :class="iso.binaryTypingProfile.Intl1 ? 'bool--pos' : 'bool--neg'"
                    >{{ iso.binaryTypingProfile.Intl1 ? '✓' : '✗' }}</span
                  >
                </td>
                <td>
                  <span
                    class="bool-badge"
                    :class="iso.binaryTypingProfile.TEM ? 'bool--pos' : 'bool--neg'"
                    >{{ iso.binaryTypingProfile.TEM ? '✓' : '✗' }}</span
                  >
                </td>
                <td>
                  <span
                    class="bool-badge"
                    :class="iso.binaryTypingProfile.SHV ? 'bool--pos' : 'bool--neg'"
                    >{{ iso.binaryTypingProfile.SHV ? '✓' : '✗' }}</span
                  >
                </td>
              </tr>
            </tbody>
            ...
            <tbody>
              <tr v-for="(seq, i) in detail.amrSequences" :key="i">
                ...
                <td>
                  <span
                    class="type-badge"
                    :class="seq.elementType === 'AMR' ? 'type--amr' : 'type--stress'"
                    >{{ seq.elementType }}</span
                  >
                </td>
                <td class="dim">{{ seq.resistanceClass }}</td>
                <td class="dim">{{ seq.subclass }}</td>
                <td style="text-align: right" class="pct-cell">
                  {{ seq.identityPercentage.toFixed(2) }}%
                </td>
                <td style="text-align: right" class="pct-cell">
                  {{ seq.coveragePercentage.toFixed(2) }}%
                </td>
              </tr>
            </tbody>
          </table>
            ...
            <tbody>
              <tr v-for="wgs in detail.wgsMetrics" :key="wgs.isolateId">
                ...
                <td style="text-align: right" class="num-cell">
                  {{ wgs.genomeLength.toLocaleString() }}
                </td>
                <td style="text-align: right" class="num-cell">
                  {{ wgs.n50Value.toLocaleString() }}
                </td>
              </tr>
            </tbody>
          </table>
...
.hero-name-btn:hover .page-title {
  color: var(--c-brand);
}
.hero-name-btn:hover .hero-chevron {
  color: var(--c-brand);
}
